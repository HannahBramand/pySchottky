...
def process_and_save_blackman_harris(csv_filepath, true_f_rev, output_name, plot_dir, data_dir):
    ...
    # Blackman-Harris Window Application
    ...
    dataset_filename = os.path.join(data_dir, f"{output_name}_BlackmanHarris.csv")
    ...
    # =========================================================================
    # PLOT 1: STANDARD FULL VIEW (Overview)
    # =========================================================================
    ...
    axs_std[2].plot(plot_win_freqs / 1e6, plot_win_amps, color='#17becf', linewidth=1.2) # Teal color for Blackman-Harris
    if detected_f_rev > 0:
        axs_std[2].plot(detected_f_rev / 1e6, peak_amp, "ro", label=f"Detected f_rev: {detected_f_rev/1e6:.4f} MHz")
        axs_std[2].legend()
    axs_std[2].set_title("3. Windowed Spectrum (Blackman-Harris Window)", fontsize=13, fontweight='bold')
    axs_std[2].set_xlabel("Frequency [MHz]")
    axs_std[2].set_ylabel("Magnitude")
    axs_std[2].grid(True, linestyle='--', alpha=0.6)
    
    plt.tight_layout()
    plot_filename_std = os.path.join(plot_dir, f"{output_name}_BlackmanHarris.png")
    fig_std.savefig(plot_filename_std, dpi=150)
    plt.close(fig_std) 

    # =========================================================================
    # PLOT 2: ZOOMED VIEW (Details)
    # =========================================================================
    ...
    axs_zoom[2].plot(plot_win_freqs[mask_win_zoom] / 1e6, plot_win_amps[mask_win_zoom], color='#17becf', linewidth=1.5, marker='.')
    if detected_f_rev > 0:
        axs_zoom[2].plot(detected_f_rev / 1e6, peak_amp, "ro", markersize=8, label=f"Peak: {detected_f_rev/1e6:.4f} MHz")
        axs_zoom[2].legend()
    axs_zoom[2].set_title("3. ZOOMED Windowed Spectrum (Blackman-Harris Window)", fontsize=13, fontweight='bold')
    axs_zoom[2].set_xlabel("Frequency [MHz]")
    axs_zoom[2].set_ylabel("Magnitude")
    axs_zoom[2].grid(True, linestyle='--', alpha=0.6)
    
    plt.tight_layout()
    plot_filename_zoom = os.path.join(plot_dir, f"{output_name}_BlackmanHarris_Zoomed.png")
    fig_zoom.savefig(plot_filename_zoom, dpi=150)
    plt.close(fig_zoom)

if __name__ == "__main__":
    print("\n==================================================")
    print("  AUTOMATED BATCH WINDOWING (BLACKMAN-HARRIS)     ")
    print("==================================================\n")
    
    # !!! IMPORTANT: PUT YOUR ABSOLUTE PATH HERE !!!
    INPUT_DIR = "/home/hannah/Hannah/all files/UniPv/Thesis/Table/new/P/time_domain/without-noise/trans+longi ( BOTH )"
    ...
    config_file = os.path.join(current_folder, "proton.txt")
    
    try:
        params = parse_proton_file(config_file)
    ...
    for p in params:
        ...
        for state in states:
            file_name = f"proton{energy}MeV_{state}_Time.csv"
            input_file = os.path.join(INPUT_DIR, file_name)
            
            output_name = f"proton{energy}MeV_{state}"
            
            if os.path.exists(input_file):
                process_and_save_blackman_harris(input_file, true_f_rev, output_name, OUTPUT_PLOT_DIR, OUTPUT_DATA_DIR)
