...
def process_and_save_blackman(csv_filepath, true_f_rev, output_name, plot_dir, data_dir):
    ...
    dataset_filename = os.path.join(data_dir, f"{output_name}_Blackman.csv")
    ...
    axs_std[2].plot(plot_win_freqs / 1e6, plot_win_amps, color='#1f77b4', linewidth=1.2)
    if detected_f_rev > 0:
        axs_std[2].plot(detected_f_rev / 1e6, peak_amp, "ro", label=f"Detected f_rev: {detected_f_rev/1e6:.4f} MHz")
        axs_std[2].legend()
    axs_std[2].set_title("3. Windowed Spectrum (Classic Blackman)", fontsize=13, fontweight='bold')
    axs_std[2].set_xlabel("Frequency [MHz]")
    axs_std[2].set_ylabel("Magnitude")
    axs_std[2].grid(True, linestyle='--', alpha=0.6)
    
    plt.tight_layout()
    plot_filename_std = os.path.join(plot_dir, f"{output_name}_Blackman.png")
    ...
    axs_zoom[2].plot(plot_win_freqs[mask_win_zoom] / 1e6, plot_win_amps[mask_win_zoom], color='#1f77b4', linewidth=1.5, marker='.')
    if detected_f_rev > 0:
        axs_zoom[2].plot(detected_f_rev / 1e6, peak_amp, "ro", markersize=8, label=f"Peak: {detected_f_rev/1e6:.4f} MHz")
        axs_zoom[2].legend()
    axs_zoom[2].set_title("3. ZOOMED Windowed Spectrum (Classic Blackman)", fontsize=13, fontweight='bold')
    axs_zoom[2].set_xlabel("Frequency [MHz]")
    axs_zoom[2].set_ylabel("Magnitude")
    axs_zoom[2].grid(True, linestyle='--', alpha=0.6)
    
    plt.tight_layout()
    plot_filename_zoom = os.path.join(plot_dir, f"{output_name}_Blackman_Zoomed.png")
    fig_zoom.savefig(plot_filename_zoom, dpi=150)
    plt.close(fig_zoom)

if __name__ == "__main__":
    print("\n==================================================")
    print("      AUTOMATED BATCH WINDOWING (BLACKMAN)        ")
    print("==================================================\n")
    ...
    for p in params:
        ...
        for state in states:
            ...
            if os.path.exists(input_file):
                process_and_save_blackman(input_file, true_f_rev, output_name, OUTPUT_PLOT_DIR, OUTPUT_DATA_DIR)
